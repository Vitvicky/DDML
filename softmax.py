...
def main():
    ...
    layer_shape = (784, 1568, 1568, 784, 196, 98)
    ...
    net = DDMLNet(layer_shape, beta=1.0, tao=3.0, b=1.0, learning_rate=0.0001)

    pkl = "pkl/ddml({}: {}-{}-{}-{}).pkl".format(test_label, layer_shape, net.beta, net.tao, net.b)
    txt = "pkl/ddml({}: {}-{}-{}-{}).txt".format(test_label, layer_shape, net.beta, net.tao, net.b)
    ...
    for si, sj in test_data_loader:
        ...
        logger.info("%6d, %2d, %2d, %9.3f, %2d", num, int(yi), int(yj), dist, prediction)

    logger.info("Softmax Classification: %.6f", softmax_currect / test_data_size)
    ...
    with open(txt, mode='a') as t:
        ...
        print('Softmax Classification: {:6.3f}'.format(softmax_currect / test_data_size), file=t)
